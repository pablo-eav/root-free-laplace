...
%   Part of the Root-Free Laplace Inversion Toolbox.
    ...
    ctrl_panel = uipanel('Parent', fig, ...
                         ...
                         'BackgroundColor', [0.95, 0.95, 0.96], ...
    ...
    grid(ax_main, 'on');
    set(ax_main, 'Color', 'w', 'XColor', 'k', 'YColor', 'k', 'FontSize', 10, 'FontWeight', 'bold', 'LineWidth', 1.1);
    title(ax_main, 'Respuesta Temporal Invertida f(z)', 'FontSize', 11, 'FontWeight', 'bold', 'Color', 'k');
    xlabel(ax_main, 'Tiempo z (s)', 'FontWeight', 'bold', 'Color', 'k');
    ylabel(ax_main, 'f(z)', 'FontWeight', 'bold', 'Color', 'k');

    ax_err = subplot(2, 1, 2, 'Parent', display_panel);
    grid(ax_err, 'on');
    set(ax_err, 'Color', 'w', 'XColor', 'k', 'YColor', 'k', 'FontSize', 10, 'FontWeight', 'bold', 'LineWidth', 1.1);
    title(ax_err, 'Diagnóstico de Error Absoluto Logarítmico', 'FontSize', 11, 'FontWeight', 'bold', 'Color', 'k');
    xlabel(ax_err, 'Tiempo z (s)', 'FontWeight', 'bold', 'Color', 'k');
    ylabel(ax_err, '|Error|', 'FontWeight', 'bold', 'Color', 'k');
    ...
    function on_preset_selected(~, ~)
        ...
        switch val
            ...
            case 3 % Cascada K=100
                num_edit.String = '1';
                den_edit.String = 'FactorPoly(-ones(100,1))';
                z_min_edit.String = '0.0';
    ...
    function on_invert_clicked(~, ~)
        try
            ...
            if startsWith(den_str, 'laplace.') || startsWith(den_str, 'FactorPoly')
            ...
            % Plot main
            cla(ax_main);
            plot(ax_main, z_grid, f_inv, 'b-', 'LineWidth', 1.8, 'DisplayName', info.engine);
            grid(ax_main, 'on');
            legend(ax_main, 'Location', 'best');
            set(ax_main, 'Color', 'w', 'XColor', 'k', 'YColor', 'k', 'FontSize', 10, 'FontWeight', 'bold', 'LineWidth', 1.1);
            title(ax_main, sprintf('Respuesta Invertida f(z) [%s]', info.engine), 'FontSize', 11, 'FontWeight', 'bold', 'Color', 'k');
            xlabel(ax_main, 'Tiempo z (s)', 'FontWeight', 'bold', 'Color', 'k');
            ylabel(ax_main, 'f(z)', 'FontWeight', 'bold', 'Color', 'k');
            ...
            if preset_popup.Value == 1
                ...
                hold(ax_main, 'on');
                plot(ax_main, z_grid, f_ref, 'k--', 'LineWidth', 1.5, 'DisplayName', 'Exacto');
                err = abs(f_inv - f_ref);
                semilogy(ax_err, z_grid, max(err, 1e-17), 'r-', 'LineWidth', 1.5);
                title(ax_err, sprintf('Error Absoluto frente a Solución Exacta (Max: %.2e)', max(err)), 'FontSize', 10, 'FontWeight', 'bold', 'Color', 'k');
            elseif preset_popup.Value == 4
                f_ref = laplace.eval_harmonic_resonators_exact(5000, z_grid);
                hold(ax_main, 'on');
                plot(ax_main, z_grid, f_ref, 'k--', 'LineWidth', 1.5, 'DisplayName', 'Núcleo Dirichlet');
                err = abs(f_inv - f_ref);
                semilogy(ax_err, z_grid, max(err, 1e-16), 'r-', 'LineWidth', 1.5);
                title(ax_err, sprintf('Error frente al Núcleo de Dirichlet (Max: %.2e)', max(err)), 'FontSize', 10, 'FontWeight', 'bold', 'Color', 'k');
            else
                plot(ax_err, z_grid, abs(f_inv), 'm-', 'LineWidth', 1.5);
                title(ax_err, 'Magnitud Absoluta |f(z)|', 'FontSize', 10, 'FontWeight', 'bold', 'Color', 'k');
            end
            grid(ax_err, 'on');
            set(ax_err, 'Color', 'w', 'XColor', 'k', 'YColor', 'k', 'FontSize', 10, 'FontWeight', 'bold', 'LineWidth', 1.1);
            xlabel(ax_err, 'Tiempo z (s)', 'FontWeight', 'bold', 'Color', 'k');
            ylabel(ax_err, 'Amplitud', 'FontWeight', 'bold', 'Color', 'k');
